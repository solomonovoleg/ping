...
**Чтобы на staging попадали изменения из нужной ветки** (например `http://130.49.150.92:3081` и клиентские маршруты вроде `/posts`):

1. Локально переключись на ветку и подтянь последнее:  
   `git checkout chore/push-2026-03-19 && git pull`
2. Запусти деплой staging (**Windows:** `npm run deploy:staging:win`, нужен `deploy.staging.env` с `PORT=3081`, `VPS_PATH`, `DATABASE_URL` и т.д.).
3. Архив на сервер собирается из **текущей рабочей копии** (активная ветка + незакоммиченные изменения внутри репо) — перед деплоем обязательно нужная ветка и при необходимости `git pull`.

Маршруты SPA (`/posts`, `/chats/…` и т.д.) отдаются через fallback на `index.html` в `server/static.ts` — отдельно nginx для `/posts` настраивать не нужно, если запрос идёт на тот же хост и порт, где крутится Node.
