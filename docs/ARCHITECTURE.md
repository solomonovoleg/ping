...
## API, токены и публичные ручки

**`docs/API_AUTH_AND_PUBLIC.md`** — как устроены сессия и Bearer `pm.*`, где на клиенте хранится токен, почему его могут «найти», различие PATCH/PUT, список публичных эндпоинтов и чеклист для новых маршрутов.
