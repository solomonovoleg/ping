...
export type MoneyPostAccrualTargetDto = { edgeId: string; threshold: number };

export async function fetchMoneyPostAccrualTargetsFromEdge(
  platformUserId: string,
): Promise<MoneyPostAccrualTargetDto[] | null> {
  const u = platformUserId.trim();
  if (!u) return [];
  const qs = new URLSearchParams({ platformUserId: u });
  try {
    const up = await fetchUpstreamEdgeServicePath(`/v1/money/post-accrual-targets?${qs}`, { method: "GET" });
    ...
    const out: MoneyPostAccrualTargetDto[] = [];
    for (const x of arr) {
