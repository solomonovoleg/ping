...
export type MoneyCallAccrualTargetDto = { edgeId: string; threshold: number };

export async function fetchMoneyCallAccrualTargetsFromEdge(platformUserId: string): Promise<MoneyCallAccrualTargetDto[] | null> {
  const u = platformUserId.trim();
  if (!u) return [];
  const qs = new URLSearchParams({ platformUserId: u });
  try {
    const up = await fetchUpstreamEdgeServicePath(`/v1/money/call-accrual-targets?${qs}`, { method: "GET" });
    ...
    const out: MoneyCallAccrualTargetDto[] = [];
    for (const x of arr) {
