...
import { MONEY_PROFILE_LIKES_TASK_KEY } from "./profile-likes-task-key.js";

export type ApplyProfileLikesMilestoneResult =
...
export async function applyProfileLikesReceivedMilestoneEvent(opts: {
  edgeId: string;
  platformUserId: string;
  blockIndex: number;
}): Promise<ApplyProfileLikesMilestoneResult | null> {
  ...
  const rule = parsed.scoringRules.find((r) => r.kind === "profile_likes_received" && r.enabled !== false);
  if (!rule || rule.points <= 0) {
    return { ok: true, awarded: false, reason: "profile_likes_rule_disabled_or_zero", xpDelta: 0 };
  }
  ...
  const now = new Date();
  if (maxDay > 0) {
    const spent = await sumXpForTaskKeyUtcDay(edgeId, user, MONEY_PROFILE_LIKES_TASK_KEY, now);
  ...
  const inserted = await tryInsertTaskGrant(edgeId, user, MONEY_PROFILE_LIKES_TASK_KEY, refKey, xpDelta);
