...
import { MONEY_VIDEO_CALL_TASK_KEY } from "./video-call-task-key.js";

export type ApplyVideoCallMilestoneResult =
...
function refKeyForMilestone(chatId: string, blockIndex: number): string {
  ...
  const key = `vc:${c}:b:${Math.floor(blockIndex)}`;
  return key.length <= 256 ? key : `vh:${blockIndex}:${c.slice(0, 200)}`;
}

export async function applyVideoCallMinutesMilestoneEvent(opts: {
  edgeId: string;
  platformUserId: string;
  chatId: string;
  blockIndex: number;
}): Promise<ApplyVideoCallMilestoneResult | null> {
  ...
  const rule = parsed.scoringRules.find((r) => r.kind === "video_call_minutes" && r.enabled !== false);
  if (!rule || rule.points <= 0) {
    return { ok: true, awarded: false, reason: "video_call_rule_disabled_or_zero", xpDelta: 0 };
  }
  ...
  const now = new Date();
  if (maxDay > 0) {
    const spent = await sumXpForTaskKeyUtcDay(edgeId, user, MONEY_VIDEO_CALL_TASK_KEY, now);
  ...
  const inserted = await tryInsertTaskGrant(edgeId, user, MONEY_VIDEO_CALL_TASK_KEY, refKey, xpDelta);
