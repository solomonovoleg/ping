...
export type MoneyProfileLikeAccrualTarget = {
  edgeId: string;
  threshold: number;
};

export async function listMoneyProfileLikeAccrualTargetsForPlatformUser(
  platformUserId: string,
): Promise<MoneyProfileLikeAccrualTarget[] | null> {
  ...
  try {
    ...
    const out: MoneyProfileLikeAccrualTarget[] = [];
    for (const r of rows) {
      ...
      const rule = parsed.scoringRules.find((x) => x.kind === "profile_likes_received" && x.enabled !== false);
      if (!rule || rule.points <= 0 || rule.threshold < 1) continue;
      out.push({ edgeId: String(r.public_id || "").trim(), threshold: rule.threshold });
    }
    return out;
  } catch (e) {
    console.error("[edge] listMoneyProfileLikeAccrualTargetsForPlatformUser", e);
    return null;
  }
}
