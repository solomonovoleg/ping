...
export type MoneyChatAccrualTarget = {
  edgeId: string;
  threshold: number;
};

/**
 * Опубликованные MONEY-кампании, где пользователь — участник, игра не на паузе по сроку/статусу,
 * и включено правило `chat_messages` с положительными баллами.
 */
export async function listMoneyChatAccrualTargetsForPlatformUser(
  platformUserId: string,
): Promise<MoneyChatAccrualTarget[] | null> {
  ...
  try {
    ...
    const out: MoneyChatAccrualTarget[] = [];
    for (const r of rows) {
      ...
      const rule = parsed.scoringRules.find((x) => x.kind === "chat_messages" && x.enabled !== false);
      if (!rule || rule.points <= 0 || rule.threshold < 1) continue;
      out.push({ edgeId: String(r.public_id || "").trim(), threshold: rule.threshold });
    }
    return out;
  } catch (e) {
    console.error("[edge] listMoneyChatAccrualTargetsForPlatformUser", e);
    return null;
  }
}
