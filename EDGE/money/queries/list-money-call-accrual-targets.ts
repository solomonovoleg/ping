...
export type MoneyCallAccrualTarget = {
  edgeId: string;
  threshold: number;
};

/** Опубликованные MONEY с правилом `video_call_minutes` (аудио и видео 1:1). */
export async function listMoneyCallAccrualTargetsForPlatformUser(
  platformUserId: string,
): Promise<MoneyCallAccrualTarget[] | null> {
  ...
  try {
    ...
    const out: MoneyCallAccrualTarget[] = [];
    for (const r of rows) {
      ...
      const rule = parsed.scoringRules.find((x) => x.kind === "video_call_minutes" && x.enabled !== false);
      if (!rule || rule.points <= 0 || rule.threshold < 1) continue;
      out.push({ edgeId: String(r.public_id || "").trim(), threshold: rule.threshold });
    }
    return out;
  } catch (e) {
    console.error("[edge] listMoneyCallAccrualTargetsForPlatformUser", e);
    return null;
  }
}
