...
import { STORE_REVIEW_CHECKLIST_ROWS } from "./checklist-data";
import type { StoreReviewCheckStatus } from "./types";

const STATUS_LABEL: Record<StoreReviewCheckStatus, string> = {
  implemented: "В продукте",
  partial: "Частично",
  manual: "Вручную / Connect",
};

const STATUS_CLASS: Record<StoreReviewCheckStatus, string> = {
...
export function ComplianceChecklistCard() {
  return (
    <AdminPanelCard className="p-5 sm:p-6">
      <h2 className="text-base font-semibold text-[hsl(210_20%_98%)]">Чеклист перед ревью Apple</h2>
      <p className="mt-1 text-sm admin-text-muted">
        Сопоставление гайдлайнов с PING. Статусы — ориентир команды, не юридическое заключение.
      </p>
      <div className="mt-4 space-y-3">
        {STORE_REVIEW_CHECKLIST_ROWS.map((row) => (
          <div
            key={row.key}
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs font-mono text-[hsl(210_12%_62%)]">Apple: {row.appleRef}</p>
              </div>
              <span
                className={cn(
