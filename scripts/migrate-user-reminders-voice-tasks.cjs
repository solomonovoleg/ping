...
"use strict";
/**
 * Напоминания и голосовые задачи ПИНГОК МИКРО.
 * migrations/0021_user_reminders_voice_tasks.sql
 */
...
const sqlPath = path.join(process.cwd(), "migrations/0021_user_reminders_voice_tasks.sql");
...
async function main() {
  ...
  try {
    ...
    console.log("user_reminders + voice_tasks ready");
  } finally {
    await client.end();
  }
}
