...
"use strict";
/**
 * Персональные настройки участника чата в списке (закрепить / скрыть / полка).
 * SQL: migrations/0019_chat_member_prefs.sql — должен выполняться на VPS (run-migrations.cjs).
 */
...
const sqlPath = path.join(process.cwd(), "migrations/0019_chat_member_prefs.sql");
...
async function main() {
  ...
  try {
    ...
    console.log("chat_member_prefs ready");
  } finally {
    await client.end();
  }
}
